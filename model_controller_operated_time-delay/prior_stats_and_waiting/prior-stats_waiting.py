...
# ):
def generate_simulation_prior_stats(rv, N1=125, N2=375, total_time=10, alpha=1, beta=1):
    lst_N1 = []
    lst_N2 = []
    lst_ratio = []
    lst_time = []
...
def generate_simulation_waiting_cars(
    rv, total_time=10, alpha=1, beta=1, num_cars_waiting=50
):
    lst_N1 = []
    lst_N2 = []
    lst_ratio = []
...
if __name__ == "__main__":
    ...
    # rv, dist = stats.uniform(loc=loc, scale=scale), "uniform"
    rv, dist = stats.expon(loc=loc, scale=scale), "exponential"
    # rv, dist = stats.chi2(df=5, loc=loc, scale=scale), "chi_sq"
    ...
    for alpha, beta in tqdm(zip([1, 1, 1, 2, 3], [1, 2, 3, 1, 1])):
        ...
        if model == "prior_stats":
            ...
            cdf, timeL, N1_l, N2_l, ratioL = generate_simulation_waiting_cars(
                rv=rv, total_time=1000, alpha=alpha, beta=beta, num_cars_waiting=20
            )
        else:
